// script.js (Randomizált sorrenddel)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /**
     * ÚJ FUNKCIÓ: Megkever egy tömböt a Fisher-Yates algoritmussal.
     */
    ...
    function showFinalMessage() {
        ...
        document.getElementById('final-heart').addEventListener('click', () => { createSingleCard(fianceeProfile, true); });
    }
    ...
    function checkIfDeckIsEmpty() {
        setTimeout(() => {
            if (cardsContainer.querySelectorAll('.card').length === 0) {
                if (localStorage.getItem('fianceeMatched') === 'true') { showGameOverMessage(); } else { showFinalMessage(); }
            }
        }, 50);
    }
    ...
    function updateMatchCounter() {
        ...
        if (!badge) { badge = document.createElement('span'); badge.className = 'notification-badge'; messagesBtn.appendChild(badge); }
        badge.style.display = matches.length > 0 ? 'block' : 'none';
    }

    function createProfileCards() {
        cardsContainer.innerHTML = ''; 
        if (localStorage.getItem('fianceeMatched') === 'true') { showGameOverMessage(); return; }
        const matches = JSON.parse(localStorage.getItem('matches')) || [];
        ...
        const profilesToDisplay = profiles.filter(profile => !matchedNames.has(profile.name) && !dislikedNames.has(profile.name));
        
        // MÓDOSÍTÁS: A megjelenítendő profilok listáját megkeverjük
        shuffleArray(profilesToDisplay);

        if (profilesToDisplay.length === 0) { showFinalMessage(); return; }
    ...
    function createSingleCard(profile, isSpecial, index = -1) {
        ...
        const imageUrl = profile.avatar ? profile.avatar : (typeof profile.images[0] === 'object' ? profile.images[0].src : profile.images[0]);
        card.style.backgroundImage = `url(${imageUrl})`;
        card.dataset.profileIndex = isSpecial ? 'special' : index;
        const displayName = profile.nickname || profile.name;
        let stampHTML = '';
        if (isSpecial) {
            stampHTML = `<div class="stamp superlike"><i class="fa-solid fa-star"></i> SUPERLIKE</div>`;
        } else {
            stampHTML = `<div class="stamp like">LIKE</div><div class="stamp nope">NOPE</div>`;
        }
        const infoHTML = `<div class="card-info"><h2>${displayName}, ${profile.age}</h2><p class="tagline">"${profile.tagline}"</p></div>`;
        card.innerHTML = stampHTML + infoHTML;
    ...
    function addCardEventListeners() {
        ...
        const onDragEnd = (e) => {
            ...
            if (Math.abs(deltaX) > decisionThreshold) {
                ...
                if (profileIndex === 'special') { setFinalMatch(); } else { const profile = profiles[parseInt(profileIndex)]; if (decision === 'like') saveMatch(profile); else saveDislike(profile); }
    ...
    function setFinalMatch() { const finalMatch = [fianceeProfile]; localStorage.setItem('matches', JSON.stringify(finalMatch)); localStorage.setItem('fianceeMatched', 'true'); updateMatchCounter(); }
    ...
    reloadBtn.addEventListener('click', () => { localStorage.removeItem('matches'); localStorage.removeItem('dislikedProfiles'); localStorage.removeItem('fianceeMatched'); createProfileCards(); updateMatchCounter(); });
